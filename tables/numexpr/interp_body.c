...
#define VEC_ARG3(expr)                          \
    ...
    unsigned int pc, j, r;
    unsigned int n_inputs = params.n_inputs;
    #define memsteps  params.memsteps
    #define memsizes  params.memsizes
    /* set up pointers to next block of inputs and outputs */
    params.mem[0] = params.output + index * memsteps[0];
    for (r = 0; r < n_inputs; r++) {
        params.mem[1+r] = params.inputs[r] + index * memsteps[1+r];
    }
    for (pc = 0; pc < params.prog_len; pc += 4) {
        ...
        intp str1 = ((arg1 <= n_inputs) ? memsteps[arg1] : memsizes[arg1]);
	...
	/* The COPY is the only op that depends on stride */
        case OP_COPY_BB:
            VEC_ARG1(memcpy(dst, src, sizeof(char));
                     dst += sizeof(char); src += str1);
        case OP_COPY_II:
            VEC_ARG1(memcpy(dst, src, sizeof(int));
                     dst += sizeof(int); src += str1);
        case OP_COPY_LL:
            VEC_ARG1(memcpy(dst, src, sizeof(long long));
                     dst += sizeof(long long); src += str1);
        case OP_COPY_FF:
        ...
        case OP_GT_BLL: VEC_ARG2(b_dest = (l1 > l2) ? 1 : 0);
        case OP_GE_BLL: VEC_ARG2(b_dest = (l1 >= l2) ? 1 : 0);
        case OP_EQ_BLL: VEC_ARG2(b_dest = (l1 == l2) ? 1 : 0);
        case OP_NE_BLL: VEC_ARG2(b_dest = (l1 != l2) ? 1 : 0);
        ...
        case OP_CAST_LB: VEC_ARG1(l_dest = (long long)(b1));
        ...
        case OP_CAST_FB: VEC_ARG1(f_dest = (double)(b1));
