...
from tables.numexpr.compiler import stringToExpression, numexpr  #XXX
...
def get_variable_namesXXX(expression):
...
def split_index_condXXX(condition, condvars, table):
    ...
    * '(0<c1) & ((c2>2) & (c1<=1))' -> ('c1',['gt'],[0],{(c2>2)&(c1<=1)},['c2','c1'])
    ...
    for colname in colnames:  # then add types of columns
        ...
        typemap[colname] = nxTypeFromColumnXXX[coldtype]
    ...
    if resexpr:
        resvarnames, ressignature = get_variable_namesXXX(resexpr), []
...
class Index(NotLoggedMixin, indexesExtension.Index, Group):
    ...
    def append(self, arr):
        ...
        sarr = arr[s]  #XYX tormar a fer un sort sembla mes rapid que usar
                       #seleccio per indexos...
        sorted.append(sarr)
        cs = self.chunksize
