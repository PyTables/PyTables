...
cdef class IndexArray(Array):
  ...
  cdef hid_t   dataset_id, type_id2, space_id, mem_space_id
  ...
  # Do not close the array while the file is opened. Keep it opened as
  # it accelerates searches. Besides, from PyTables 1.2 on, this will
  # be close when the index would be pre-empted from cache.
  def _closeIndexSlice(self):
    if self.isopen_for_read:
      if H5ARRAYOclose_readSlice(self.dataset_id, self.space_id,
                                 self.type_id2) < 0:
        raise RuntimeError("Problems closing the index array data.")
  ...
  def _initSortedSlice(self, pro=0):
    ...
    if self.bufferl is None:
      ...
      # Open the sorted array for reading
      if (H5ARRAYOopen_readSortedSlice(&self.dataset_id, &self.space_id,
  ...
  # Do not close the array while file is openend. Keep it opened as it
  # accelerates searches. Besides, from PyTables 1.2 on, this will be
  # close when the index would be pre-empted from cache.
  def _closeSortedSlice(self):
    # Close the array for reading if needed
    if self.isopen_for_read:
      if H5ARRAYOclose_readSortedSlice(self.dataset_id, self.space_id,
                                       self.mem_space_id, self.type_id2) < 0:
